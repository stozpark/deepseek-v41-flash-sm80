...
if not cuda_version.startswith("13.0"):
    raise SystemExit(
        f"ERROR: main/cu130 SIF must use CUDA 13.0 PyTorch runtime, got {cuda_version!r}"
    )
...
kv_cache = torch.zeros((1, BLOCK_SIZE, 1, D + 4), dtype=torch.uint8, device=device)
...
actual = torch.empty(NUM_REQS, 1, HEAD_DIM, dtype=torch.bfloat16, device=device)
expected = torch.empty_like(actual)
dequantize_and_gather_k_cache(
    actual, k_cache, seq_lens, None, block_table_view, GATHER_BLOCK_SIZE, 0
)
dequantize_and_gather_k_cache(
    expected,
    k_cache,
...
for needle, text, label in (
    ...
    if needle not in text:
        raise SystemExit(f"ERROR: V4.1 SWA guard missing at {label}")
print("multimodal_swa_guards: OK")
...
print("SM80 DeepSeek-V4.1 CUDA 13 runtime verification: OK")
