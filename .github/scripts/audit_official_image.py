...
# This workflow runs on a GPU-less GitHub runner. Do not import DeepSeek V4.1
# GPU modules here: vLLM intentionally disables Triton when no active driver is
# present, which makes import-only checks fail for reasons unrelated to source
# compatibility. Physical import/kernel validation belongs in verify_runtime.py
# on the A100. Here we validate the exact official-image source tree after the
# SM80 overlay: syntax, lazy registry presence, routing invariants, and the
# cross-file API contracts that previously caused import failures.
...
source_files = [
    "model_executor/kernels/linear/gemv_triton.py",
...
# DeepSeek V4.1 reuses pieces of the V4 implementation (notably the MoE base
# and NVIDIA projection helpers), so keep source-level guards for the shared
# code that the V4.1 import graph references. These checks do not make V4 the
# target model; they protect V4.1's shared dependency closure.
...
print("OFFICIAL DEEPSEEK-V4.1-FLASH IMAGE COMPATIBILITY AUDIT OK")
