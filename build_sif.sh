...
PATCH_FILE="${PATCH_DIR}/sm80-cu129-minimal.patch"
PATCH_SHA="${PATCH_DIR}/sm80-cu129-minimal.patch.sha256"
PATCH_MANIFEST="${PATCH_DIR}/sm80-cu129-minimal.manifest.txt"
...
grep -qx 'From: vllm/vllm-openai:deepseekv41-flash-0909-cu129' "$ONLINE_DEF" || {
...
validate_cuda129_sif() {
  local sif="$1"
  echo "[check] validating CUDA 12.9 runtime in SIF: $sif"
...
if not cuda.startswith("12.9"):
    print(f"ERROR: expected CUDA 12.9 runtime, got {cuda!r}", file=sys.stderr)
    raise SystemExit(1)
'
}
...
if [[ -n "$BASE_SIF_INPUT" ]]; then
  ...
  validate_cuda129_sif "$BASE_SIF_INPUT"
